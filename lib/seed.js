...
const sampleData = Array.from({ length: 40}, (_, i) => ({
    ...
    unit: carUnits[i],
    pickup: `2025-7-${(i % 30 + 1).toString().padStart(2, "0")}`,
    return: `2025-8-${(i % 30 + 1).toString().padStart(2, "0")}`,
    status: ["Approved", "Rejected", "Pending"][i % 3],
}))

const insert = db.prepare(
    `INSERT INTO listings(booking_id, client, phone, unit, pickup, return, status)
    VALUES (@booking_id, @client, @phone, @unit, @pickup, @return, @status)`
);
